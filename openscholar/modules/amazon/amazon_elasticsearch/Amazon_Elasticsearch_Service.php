...
class AmazonElasticsearchService extends DrupalApacheSolrService {
  ...
  public function ping($timeout = 2) {
    ...
    if ($response->code == 200) {
      ...
      drupal_set_message('200 response. timeout check');
  ...
  protected function sign($verb, $uri, $query_string, $body = "") {
    ...
    // signing key
    $keyDate = hash_hmac("SHA256", $date, "AWS4$secret_key", true);
    $keyRegion = hash_hmac("SHA256", 'us-east-1', $keyDate, true);
    $keyService = hash_hmac("SHA256", 'cloudsearch', $keyRegion, true);
