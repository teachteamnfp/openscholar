...
class VsiteCpSettingsTest extends OsExistingSiteTestBase {
  ...
  public function testCpSettingsPageFormSave(): void {
    ...
    $this->visit("{$this->group->get('path')->first()->getValue()['alias']}/cp/settings/global-settings/vsite");
    $this->assertSession()->statusCodeEquals(200);
    $html = $this->getSession()->getPage()->getContent();
    $this->assertContains('Choose how long lists of content will display', $html);
    $this->drupalPostForm("{$this->group->get('path')->first()->getValue()['alias']}/cp/settings/global-settings/vsite", [
