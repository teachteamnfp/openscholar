...
if git show-ref -q --verify refs/tags/$CI_BRANCH 2>&1 > /dev/null; then
  ...
  git clone -b mp-dev-test git@bitbucket.org:openscholar/deploysource.git  ~/src/amazon;
  cd ~/src/amazon
...
echo $(git log --grep="^git-subtree-dir: openscholar/*\$" --no-show-signature --pretty=format:'START %H%n%s%n%n%b%nEND%n')
